...
dashboard = function () { // ini
  ...
  if (TB.user) {
    dashboard.user = TB.user.toLowerCase()
  }
  ...
  if (dashboard.user && !dashboard.isExpired(TB.exp)) {
    ...
    $.getScript('./js/jwt-decode.min.js')
      .then(function () {
        ...
        TB.alternateName = decodedToken.exp;
...
dashboard.tsv2json = function (x) {
  ...
  rows.slice(1).forEach(function (r, i) {
    ...
    console.log(yi.Entity + yi.Attribute + yi.Value + email + email + "ssss" + dashboard.user);

    if (yi.Attribute == 'tableauUsername' && yi.Entity == email) {
      dashboard.usernameAlternate = yi.Value
      //alert(yi.Value+ dashboard.usernameAlternate)

    }

    if (yi.Entity.match('@')) {
      yi.Entity.toLowerCase()
    } // emails forced to lowercase
    // indexed y
    if (!y[yi.Entity]) {
      y[yi.Entity] = {}
    }
    if (!y[yi.Entity][yi.Attribute]) {
      y[yi.Entity][yi.Attribute] = []
    }
...
dashboard.getDashboardsForUser = function (email) {
  ...
  // console.log(email+JSON.stringify(dashboard.dt))
  if (!dashboard.dt[email]) {
    dashboard.dt[email] = {
      dashboard: []
    } // empty array if none found for this user
  }
  ...
  // console.log(dd+"$$$"+typeof dd+ email);
  dd.forEach(function (d) {
    var x = dashboard.dt[d]
    if (!y[d]) {
      y[d] = {};
      y[d].more = {}
    } // register object only if it didn't exist already - so new attributes can also be appended 
...
dashboard.setBookmarked = function () {
  if (!localStorage.dashboardBookmarks) {
    localStorage.setItem('dashboardBookmarks', '[]')
  };
...
dashboard.UI = function () {
  ...
  document.getElementById("dashboardUserAlternate").innerHTML = dashboard.usernameAlternate;
...
dashboard.onclickBookmark = function (obj) {
  ...
  var fa = $('.fa', div)[0]
  fa.onclick = function () {
    dashboard.onclickUnBookmark(this)
  }
  setTimeout(function () {
...
dashboard.onclickUnBookmark = function (obj) {
  ...
  var fa = $('.fa', div)[0]
  fa.onclick = function () {
    dashboard.onclickBookmark(this)
  }
  setTimeout(function () {
...
dashboard.initializeViz = function () {
  ...
  if (dashboard.usernameAlternate) {
  ...
  console.log('url - ' + dashboard.usernameAlternate);
